...
export interface GameState {
  ...
  feed: FeedEvent[];
  pnrSwitched?: boolean;
  // The active ball-screen "call" — COORDINATION ONLY: which handler called for a pick
  // and which teammate is the chosen screener (+ when, for the lifecycle). It carries
  // NO geometry: the screener decides WHERE to set the pick himself, each tick, by
  // anchoring on the live on-ball defender in his own off-ball decider (no central
  // spot to go stale). Cleared when the pick is used / expires / the possession
  // changes. Null = no live call.
  screenCall?: { screener: Player; handler: Player; startClock: number } | null;
